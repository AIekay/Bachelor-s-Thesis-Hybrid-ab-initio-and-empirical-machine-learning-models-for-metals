"""
Molecular dynamics for solid Cu starting from an FCC configuration.
Runs NPT ensemble; results saved under RESULTS/SOLID/.
"""
...
parser = argparse.ArgumentParser(description="Solid Cu NPT MD simulation")
...
results_log = os.path.join(base_dir, "RESULTS", "SOLID", "logs")
results_xyz = os.path.join(base_dir, "RESULTS", "SOLID", "xyz")
...
xyz_out   = os.path.join(results_xyz, f"solid_{T}K.xyz")
log_out   = os.path.join(results_log, f"solid_{T}K.log")
final_out = os.path.join(results_log, f"solid_{T}K_final.lammps-data")

# FCC copper supercell (3x3x3)
a = 3.615  # Å
atoms = bulk("Cu", "fcc", a=a, cubic=True) * (3, 3, 3)
...
print(f"Starting solid MD at {T} K")
dyn = NPT(
    atoms,
    timestep,
    temperature_K=T,
